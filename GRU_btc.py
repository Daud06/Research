...
def save_model_in_multiple_formats(model, model_name, save_dir='saved_models'):
    ...
    # Also save as 'best_gru_model.keras' in root directory for easy access
    ...
    # 4. Save weights only - Use .weights.h5 extension
...
# ==========================================
def evaluate_model(model, X_test, y_test, train_max, model_name="GRU"):
    """
...
def plot_predictions(results, save_dir='saved_models'):
    ...
    ax1.plot(pred_flat, label='GRU Prediction', alpha=0.7, linewidth=1)
    ax1.set_title(f'GRU Model: Actual vs Predicted (MAE: {results["mae"]:.4f})')
    ...
    ax3.bar(hours, results['per_step_mae'], alpha=0.7, color='green')
    ...
    ax4.hist(errors, bins=50, alpha=0.7, color='green', edgecolor='black')
    ...
    # Save plot
    plot_path = os.path.join(save_dir, 'gru_predictions_plot.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"✅ Plot saved as: {plot_path}")

    # Also save in root directory
    plt.savefig('gru_predictions_plot.png', dpi=300, bbox_inches='tight')
...
def plot_training_history(history, save_dir='saved_models'):
    ...
    # Save plot
    plot_path = os.path.join(save_dir, 'gru_training_history.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.show()
...
if __name__ == "__main__":
    ...
    callbacks = [
        ...
        # Save best model during training - this creates 'best_gru_model.keras'
        ModelCheckpoint('best_gru_model.keras',
                        monitor='val_loss',
                        save_best_only=True,
                        verbose=1,
                        mode='min'),
        # Also save checkpoint in saved_models directory
        ModelCheckpoint(os.path.join(SAVE_DIR, 'best_gru_checkpoint.keras'),
                        monitor='val_loss',
                        save_best_only=True,
                        verbose=0,
                        mode='min')
    ...
    # Save final model in multiple formats (this will also save as 'best_gru_model.keras')
    saved_paths = save_model_in_multiple_formats(model, 'gru_model', save_dir=SAVE_DIR)
    ...
    results = evaluate_model(model, X_test, y_test, train_max, "GRU")
    ...
    results_csv_path = os.path.join(SAVE_DIR, 'gru_model_results.csv')
    ...
    per_hour_csv_path = os.path.join(SAVE_DIR, 'gru_per_hour_mae.csv')
    ...
    predictions_csv_path = os.path.join(SAVE_DIR, 'gru_predictions.csv')
    ...
    print("FINAL SUMMARY")
    print("=" * 60)
    print(f"Model Type: GRU (Gated Recurrent Unit)")
    ...
    # GRU-specific advantages
    print(f"\nGRU Advantages:")
    print(f"  • Fewer parameters than LSTM (faster training)")
    print(f"  • Less prone to overfitting on smaller datasets")
    print(f"  • Similar performance to LSTM with less complexity")
    print(f"  • Typically trains 20-30% faster than LSTM")

    print("\n" + "=" * 60)
    print("✅ GRU MODEL TRAINING, EVALUATION, AND SAVING COMPLETED SUCCESSFULLY!")
    print("=" * 60)
    ...
    print("\nRoot directory:")
    root_files = ['best_gru_model.keras', 'gru_model_results.csv',
                  'gru_predictions_plot.png', 'gru_training_history.png']
    ...
    print("\n" + "=" * 60)
    print("IMPORTANT: Model saved as 'best_gru_model.keras'")
    ...
    sample_actual = y_test[sample_idx] * train_max
    sample_pred = model.predict(sample_input, verbose=0)[0] * train_max

    print(f"\nSample forecast (first 12 hours):")
    print(f"{'Hour':<6} {'Actual':<12} {'Predicted':<12} {'Error':<12}")
    print("-" * 45)
    for i in range(12):
        error = sample_pred[i] - sample_actual[i]
        print(f"{i + 1:02d}    {sample_actual[i]:<12.2f} {sample_pred[i]:<12.2f} {error:<12.2f}")