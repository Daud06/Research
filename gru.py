...
# ==========================================
def save_model_in_multiple_formats(model, model_name, save_dir='saved_models_gru'):
...
def save_model_checkpoints(model, history, results, X_test, y_test, save_dir='saved_models_gru'):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
# ==========================================
def plot_predictions(results, save_dir='saved_models_gru'):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    axes[0,0].plot(pred_flat, label='GRU Prediction', alpha=0.7, linewidth=1)
    axes[0,0].set_title(f'GRU: Actual vs Predicted (MAE: {results["mae"]:.4f})')
    ...
    axes[1,0].bar(days, results['per_step_mae'], alpha=0.7, color='green')
    ...
    plt.tight_layout()
    plot_path = os.path.join(save_dir, 'gru_sp500_predictions.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"✅ Plot saved as: {plot_path}")
    plt.savefig('gru_sp500_predictions.png', dpi=300, bbox_inches='tight')

def plot_training_history(history, save_dir='saved_models_gru'):
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    ...
    plt.tight_layout()
    plot_path = os.path.join(save_dir, 'gru_sp500_training_history.png')
    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"✅ Training history plot saved as: {plot_path}")
    plt.savefig('gru_sp500_training_history.png', dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    callbacks = [
        ...
        ModelCheckpoint('best_gru_sp500.keras', monitor='val_loss', save_best_only=True, verbose=1, mode='min'),
        ModelCheckpoint(os.path.join(SAVE_DIR, 'best_gru_sp500_checkpoint.keras'), monitor='val_loss', save_best_only=True, verbose=0, mode='min')
    ]
    ...
    print("="*60)
    saved_paths = save_model_in_multiple_formats(model, 'gru_sp500', save_dir=SAVE_DIR)
    ...
    results_df.to_csv(os.path.join(SAVE_DIR, 'gru_sp500_results.csv'), index=False)
    results_df.to_csv('gru_sp500_results.csv', index=False)

    per_day_df = pd.DataFrame({'Day': range(1, STEPS_AHEAD+1), 'MAE': results['per_step_mae']})
    per_day_df.to_csv(os.path.join(SAVE_DIR, 'gru_sp500_per_day_mae.csv'), index=False)
    ...
    predictions_df.to_csv(os.path.join(SAVE_DIR, 'gru_sp500_predictions.csv'), index=False)
    ...
    print("FINAL SUMMARY")
    print("="*60)
    print(f"Model Type: GRU (daily S&P 500)")
    ...
    print("\n" + "="*60)
    print("✅ GRU MODEL FOR S&P 500 COMPLETED SUCCESSFULLY!")
    print("="*60)
    ...
    print("-" * 60)
    print("\nRoot directory:")
    root_files = ['best_gru_sp500.keras', 'gru_sp500_results.csv', 'gru_sp500_predictions.png', 'gru_sp500_training_history.png']
